...
for PRD, task, design document, and objective content. The SQLite database
...
Directory structure:
    ...
    └── {project}/
        ...
        └── objectives/
            └── {run-id}.md     # Objective file (goal loop progress)
"""
...
{traces_section}{design_section}"""
    ...
    def get_objectives_dir(self, project_id: str) -> Path:
        """Get objectives directory for a project.

        Args:
            project_id: Project identifier

        Returns:
            Path to objectives directory (created if it does not exist)
        """
        obj_dir = self.base_path / project_id / "objectives"
        obj_dir.mkdir(parents=True, exist_ok=True)
        return obj_dir

    def get_objective_path(self, project_id: str, run_id: str) -> Path:
        """Get path for an objective markdown file.

        Args:
            project_id: Project identifier
            run_id: Execution run identifier (e.g., "SDLC-R0042")

        Returns:
            Path to objective markdown file
        """
        return self.get_objectives_dir(project_id) / f"{run_id}.md"

    def create_objective_file(
        self,
        project_id: str,
        run_id: str,
        description: str,
    ) -> Path:
        """Create an objective file with initial template content.

        Creates the objectives directory if it does not exist, then writes
        a markdown file containing the objective description, run metadata,
        and placeholder sections for acceptance criteria and iteration history.

        Args:
            project_id: Project identifier (e.g., "a-sdlc")
            run_id: Execution run ID (e.g., "SDLC-R0042")
            description: Human-readable objective description

        Returns:
            Path to the created objective file

        Raises:
            OSError: If the directory cannot be created or the file cannot
                be written (e.g., permission denied)
        """
        file_path = self.get_objective_path(project_id, run_id)
        content = (
            ...
            f"\n"
            f"(Generated by orchestrator in Phase 1)\n"
            f"\n"
            f"## Iteration History\n"
            f"\n"
        )
    ...
    def read_objective(self, project_id: str, run_id: str) -> str | None:
        """Read objective file content.
        ...
        Returns:
            Objective content as string, or None if not found
        """
        file_path = self.get_objective_path(project_id, run_id)
        return self.read_content(file_path)

    def list_objective_files(self, project_id: str) -> list[Path]:
        """List all objective files for a project.

        Args:
            project_id: Project identifier

        Returns:
            Sorted list of objective file paths
        """
        obj_dir = self.base_path / project_id / "objectives"
        if not obj_dir.exists():
            return []
        return sorted(obj_dir.glob("*.md"))

    def delete_objective(self, project_id: str, run_id: str) -> bool:
