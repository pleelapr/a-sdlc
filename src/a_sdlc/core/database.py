...
class Database:
    ...
    def _migrate_v8_to_v9(self, conn: sqlite3.Connection) -> None:
        """Migrate database from version 8 to version 9 (add audit_log table)."""
        conn.executescript("""
            -- Append-only audit log
    ...
    def _migrate_v9_to_v10(self, conn: sqlite3.Connection) -> None:
        """Migrate database from version 9 to 10 (no-op, removed tables cleaned up)."""
        conn.execute("UPDATE schema_version SET version = 10")

    def _migrate_v10_to_v11(self, conn: sqlite3.Connection) -> None:
        """Migrate database from version 10 to 11 (no-op, removed tables cleaned up)."""
    ...
    def _migrate_v12_to_v13(self, conn: sqlite3.Connection) -> None:
        """Migrate database from version 12 to 13 (no-op, removed tables cleaned up)."""
        conn.execute("UPDATE schema_version SET version = 13")

    def _migrate_v13_to_v14(self, conn: sqlite3.Connection) -> None:
        """Migrate database from version 13 to 14 (no-op, removed tables cleaned up)."""
        conn.execute("UPDATE schema_version SET version = 14")

    def _migrate_v14_to_v15(self, conn: sqlite3.Connection) -> None:
        """Migrate database from version 14 to 15 (no-op, removed tables cleaned up)."""
    ...
    def get_next_task_id(self, project_id: str) -> str:
        ...
        with self.connection() as conn:
            cursor = conn.execute(
                """SELECT MAX(CAST(SUBSTR(id, ?) AS INTEGER))
                   FROM tasks WHERE project_id = ? AND id LIKE ?""",
                (len(prefix) + 1, project_id, f"{prefix}%"),
            )
    ...
    def get_next_sprint_id(self, project_id: str) -> str:
        ...
        with self.connection() as conn:
            cursor = conn.execute(
                """SELECT MAX(CAST(SUBSTR(id, ?) AS INTEGER))
                   FROM sprints WHERE project_id = ? AND id LIKE ?""",
                (len(prefix) + 1, project_id, f"{prefix}%"),
            )
            max_num = cursor.fetchone()[0] or 0
    ...
    def get_next_prd_id(self, project_id: str) -> str:
        ...
        with self.connection() as conn:
            cursor = conn.execute(
                """SELECT MAX(CAST(SUBSTR(id, ?) AS INTEGER))
                   FROM prds WHERE project_id = ? AND id LIKE ?""",
                (len(prefix) + 1, project_id, f"{prefix}%"),
            )
            max_num = cursor.fetchone()[0] or 0
